...
{
  for (auto element : listOfTokens)
  {
    //cout<<"Line: " + element.lineNum<<" ";
    cout<< element.tokenInstance<<endl;
  } 
}

list <Token> Scanner::returnListOfTokens()
{
  return listOfTokens;
}

Token Scanner::getToken( )// takes in a token and prints its instance, token id, and line number
{

      Token newToken;
      newToken = listOfTokens.front();
...
Token Scanner::scanToken(const std::string input, int line)//takes in string and with each character goes through the fsa table to find the token id
{
  
  string str = "";
  int row = 0, col = 0,entry;
  ...
  while(stringChar[x] != '\0')
  {
    ...
    if (col == -1)
    {
      cout << "Error invalid token " << str << " Line: " << (lineNum + 1) <<endl;
      exit(1);
    }
